...
class ProxyInstance(val profile: Profile, private val route: String = profile.route) {
    ...
    fun start(service: BaseService.Interface, stat: File, configFile: File, mode: String, dnsRelay: Boolean = true) {
        ...
        config.put("locals", JSONArray().apply {
            ...
            Log.e("local_address", DataStore.listenAddress)
            Log.e("local_port", ""+DataStore.portProxy)
            Log.e("local_udp_address", DataStore.listenAddress)
            Log.e("local_udp_port",""+ DataStore.portProxy)
            Log.e("mode", mode)
            // local DNS proxy
            if (dnsRelay) try {
                ...
                Log.e("remote_dns_address", dns.host ?: "0.0.0.0")
                Log.e("remote_dns_port",""+ dns.port)
                put(JSONObject().apply {
                    put("local_address", DataStore.listenAddress)
        ...
        Log.e("local_address", DataStore.listenAddress)
        Log.e("local_port", ""+DataStore.portLocalDns)
        Log.e("local_dns_address", "local_dns_path")
